...
async function getSLADueDates(priority) {
  ...
  return {
    slaResponseDue: addMinutes(
      now,
      slaConfiguration.responseTimeMinutes
    ),

    slaResolutionDue: addMinutes(
      now,
      slaConfiguration.resolutionTimeMinutes
    ),
  };
}
...
async function createTicket(data, creatorId) {
  ...
  const slaDates = await getSLADueDates(
    priority
  );
  ...
  logger.info(
    `Ticket created: ${ticketNumber} by ${creatorId}`
  );
...
async function changeStatus(
  ...
  if (
    !transitions[ticket.status]?.includes(status)
  ) {
    throw new AppError(
...
async function deleteComment(
  ...
  const comment =
    await prisma.ticketComment.findFirst({
      where: {
        id: commentId,
        ticketId,
        deletedAt: null,
...
async function addAttachment(
  ...
  const url =
    `/api/${apiVersion}/tickets/${ticketId}`
...
async function getAttachmentForDownload(
  ...
  if (
    path.dirname(filePath) !== uploadRoot
